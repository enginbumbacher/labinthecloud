...
module.exports = {
  ...
  update: (config) => {
    const v_eye = EuglenaUtils.transform_roll_pitch_yaw(config.last.roll, 0, config.last.angle, { x: 0, y: 1, z: 0 });
    const v_head = EuglenaUtils.transform_roll_pitch_yaw(config.last.roll, 0, config.last.angle, { x: 1, y: 0, z: 0 });
    ...
    out.y = config.last.y + Math.sin(out.angle) * config.track.oneEye.v * dT

...
export default class LabFormModule extends Module {
  ...
  _onSandbox(evt) {
    ...
    cleanup.then(() => {
      ...
      conf.experiment.allowNames = true;
  ...
  _onSaveExperimentRequest(evt) {
    ...
    if (exp) {
      ...
      Globals.get('Relay').dispatchEvent('Notifications.Add', {
        ...
        message: `Added experiment (${exp.name}) to experiment presets`,
        autoExpire: 10
      })
    } else {
      Globals.get('Relay').dispatchEvent('Notifications.Add', {
        app: this._preview.name(),
        id: 'experiment_preset_save_error',
        type: 'error',
        message: `You must submit the experiment before adding it to the presets`,
        autoExpire: 10
      })
    }
  }
  ...
  _onSaveModelARequest(evt) {
    this._saveModelRequest(0);
  }
  _onSaveModelBRequest(evt) {
    this._saveModelRequest(1);
  }

  _saveModelRequest(ind) {
    let key = String.fromCharCode(97 + ind);
    let modelData = this._preview.getModel(key);
    if (!modelData) {
      return;
    }
    (modelData.simulated ? this._preview.triggerModelSave(key) : Promise.resolve(true)).then(() => {
      let m = this._preview.getModel(key);
      this._form.getField('model').addToHistory(m, ind);
      Globals.get('Relay').dispatchEvent('Notifications.Add', {
        app: this._preview.name(),
        id: `model_${key}_preset_save`,
        message: `Added model (${m.name}) to Model ${key.toUpperCase()} presets`,
        autoExpire: 10
      })
    }, (err) => {
      console.log(err);

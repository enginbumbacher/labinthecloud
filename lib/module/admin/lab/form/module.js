...
export default class LabFormModule extends Module {
  ...
  _onSaveExperimentRequest(evt) {
    this._form.getField('experiment').addToHistory(this._preview.getExperiment());
  }
  _onSaveModelARequest(evt) {
    ...
    if (modelData.simulated) {
      this._preview.triggerModelSave('a').then(() => {
        this._form.getField('model').addToHistory(this._preview.getModel('a'), 0);
      }, (err) => {
        console.log(err);
      });
    } else {
  ...
  _onSaveModelBRequest(evt) {
    ...
    if (modelData.simulated) {
      this._preview.triggerModelSave('b').then(() => {
        this._form.getField('model').addToHistory(this._preview.getModel('b'), 1);
      });
    } else {

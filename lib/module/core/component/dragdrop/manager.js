...
export default class DragDropManager extends Component {
  ...
  destroy() {
    ...
    let proms = []
    proms = proms.concat(Object.values(this._model.get('dragItems')).map((di) => di.destroy()));
    proms = proms.concat(Object.values(this._model.get('dropSites')).map((ds) => ds.destroy()));
    proms.push(super.destroy());

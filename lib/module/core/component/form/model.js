...
export default class FormModel extends Model {
  constructor(settings) {
    if (settings.data.fields) {
      ...
      if (settings.data.regionNames) {
        for (let regionName of Object.keys(settings.data.regionNames)) {
          settings.data.regions[regionName] = []
        }
        settings.data.regions['default'] = []

        for (let field of settings.data.fields){
          if (field._model._data.region) {
            settings.data.regions[field._model._data.region].push(field)
          } else {
            settings.data.regions['default'].push(field)
          }
        }
      } else {

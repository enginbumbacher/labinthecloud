...
export default class FormView extends DomView {
  ...
  render(model) {
    ...
    if (Object.keys(model.get('regions')).length>1) {
      ...
      // Order regions according to position list
      let regionNames = null;
      if (Object.keys(model.get('regionNames').length>0)){
        regionNames = model.get('regionNames')
        for (let region of Object.keys(model.get('regionNames'))) {
          regions[model.get('regionNames')[region]['position']] = region
        }
      }

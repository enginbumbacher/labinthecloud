...
class MultiField extends Field {
  ...
  _onAddRequest(evt) {
    evt.stopPropagation();
    if (this._model.get('addOverride')) {
      this.dispatchEvent('MultiField.AddOverride', {});
    } else {
...
MultiField.create = (data = {}) => {
  return new MultiField({ modelData: data })
}

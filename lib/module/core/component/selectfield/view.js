...
export default class SelectFieldView extends BaseFieldView {
  constructor(model, tmpl) {
    ...
    if (model.get('multiple')) this.$el.find('.selectfield__select').prop('multiple', true);
    ...
    if (model.get('standardizeFormat')) {
      this.$el.find(".selectfield__label").parent().css('display','flex');
      this.$el.find(".selectfield__label").parent().css('width','100%');
      this.$el.find(".selectfield__label").parent().css('padding','0px 5px');
      this.$el.find(".selectfield__label").parent().css('justify-content','flex-end');
      this.$el.find(".selectfield__label").css('display','inline-block');
      this.$el.find(".selectfield__label").css('flex-grow','1');
      this.$el.find(".selectfield__label").css('font-size','14px');
      this.$el.find(".selectfield__select").css('display','inline-block');
      this.$el.find(".selectfield__select").css('margin','0 0.5rem');
      this.$el.find(".selectfield__select").css('min_width','120px');
    }
  }
    ...
    for (let id in model.get('options')) {
      ...
      if (!this._options[id]) {
        this._options[id] = new OptionView({
          id: id,
          label: label,
          selected: model.get('multiple') ? model.value().includes(id) : model.value() == id
        });
        this.addChild(this._options[id], ".selectfield__select");
      } else {
        this._options[id].select(model.get('multiple') ? model.value().includes(id) : model.value() == id);
      }

...
import Template from './sliderfield.html';
import DomView from 'core/view/dom_view';
import $ from 'jquery';

import './style.scss';

export default class SliderFieldView extends FieldView {
  constructor(model, tmpl) {
    ...
    if (model.get('color')) this.$el.find('.sliderfield__label').css('color', model.get('color'));
    if (model.get('min_width')) this.$el.find('.sliderfield__content').css('min-width',model.get('min_width'));
    this.render(model);
  }

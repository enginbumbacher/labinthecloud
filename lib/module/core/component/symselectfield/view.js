...
import Template from './symselectfield.html';
import $ from 'jquery';

import './style.scss';

export default class SymSelectFieldView extends BaseFieldView {
  constructor(model, tmpl) {
    ...
    if (model.get('min_width')) this.$el.find('.symselectfield__select').css('min-width',model.get('min_width'));
    //if (model.get('inverse_order')) { this.$el.find(".symselectfield__label").remove().insertAfter(this.$el.find(".symselectfield__select"));}

    if (!Utils.exists(model.get('varOptions')) || !Object.keys(model.get('varOptions')).length) {

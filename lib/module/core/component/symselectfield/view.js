  ...
  return class SymSelectFieldView extends BaseFieldView {
    constructor(model, tmpl) {
      ...
      if (!model.get('includeVariation')) {
        ...
        if (!(model.get('id')==='variation')) { this.$el.find(".symselectfield__select").css({'margin-right': 'calc(0.5rem + 120px)'}); }
        else {
    ...
    _render(model) {
      ...
      for (let id in model.get('options')) {
        let label = null;
        if (parseInt(id + 1)) { // if no key is given in the options, but just an array of values

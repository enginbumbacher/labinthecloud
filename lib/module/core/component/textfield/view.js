...
export default class TextFieldView extends BaseFieldView {
  ...
  render(model) {
    ...
    if (model.get('prefix')) {
      this.$el.find('.textfield__prefix .textfield__meta').html(model.get('prefix')).parent().show();
    } else {
      this.$el.find('.textfield__prefix .textfield__meta').html('').parent().hide();
    }
    if (model.get('suffix')) {
      this.$el.find('.textfield__suffix .textfield__meta').html(model.get('suffix')).parent().show();
    } else {
      this.$el.find('.textfield__suffix .textfield__meta').html('').parent().hide();
    }

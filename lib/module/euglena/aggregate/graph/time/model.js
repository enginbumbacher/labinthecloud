define((require) => {
  ...
  return class AggregateTimeGraphModel extends Model {
    constructor(conf) {
      ...
      super(conf);
      Utils.bindMethods(this, ['_onGroupChange'])
    }
    ...
    update(datasets) {
      ...
      if (group.get('experiment')) {
        this._update(group)
      } else {
        group.addEventListener('Model.Change', this._onGroupChange)
      }
    }
    ...
    _onGroupChange(evt) {
      if (evt.data.path == "experiment") {
        evt.currentTarget.removeEventListener('Model.Change', this._onGroupChange);
        this._update(evt.currentTarget);
      }
    }

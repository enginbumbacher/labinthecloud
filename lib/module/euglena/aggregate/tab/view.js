...
export default class AggregateTabView extends DomView {
  ...
  _onTabClick(jqevt) {
    ...
    if (!this.tabStatus) {
      this.tabStatus = 'open';
      this.enableFields();
    } else if (this.tabStatus == 'closed') {
      this.tabStatus = 'open';
      this.enableFields();
    } else if (this.tabStatus == 'open') {
      this.disableFields();
    }
  }
  ...
  toggle(tabOpen) {
    if (tabOpen) {
      this.$el.find('#selectAggregate').removeClass('notflippedY');
      this.$el.find('#selectAggregate').addClass('flippedY');
    } else {
      this.$el.find('#selectAggregate').removeClass('flippedY');
      this.$el.find('#selectAggregate').addClass('notflippedY');
    }
  }
}

define((require) => {
  ...
  return class CircleHistogramView extends DomView {
    ...
    setup(model) {
      this.arc = d3.arc()
        .outerRadius((d) => {
          console.log(d.data.frequency, (model.get('width') / 2) * (d.data.frequency / model.get('data.maxBinValue')));
          return Utils.roundDecimal((model.get('width') / 2) * (d.data.frequency / model.get('data.maxBinValue')), -2)
        })
        // .outerRadius(1)
        .innerRadius(0);
      this.pie = d3.pie()

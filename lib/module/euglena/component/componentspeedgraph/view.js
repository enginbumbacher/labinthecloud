...
import Template from './componentspeedgraph.html';
import './componentspeedgraph.scss';
...
export default class ComponentSpeedView extends DomView {
      ...
      for (let layer in model.get('data')) {
        ...
        if (model.get('stdBand')) {
          ...
          this.svgs[key].append('path')
            ...
            .attr('style', color ? `fill: #${pad(color.toString(16),6)}` : null)
            .attr('d', std);
        }
        ...
        this.svgs[key].append('path')
          ...
          .attr('style', color ? `stroke: #${pad(color.toString(16),6)}` : null)
          .attr('d', line);

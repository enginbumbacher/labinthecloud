  ...
  return class EuglenaModel extends Model {
    ...
    getState(time, bounds) {
      ...
      return {
        ...
        yaw: sample.yaw,  //sample.yaw + (next.yaw - sample.yaw) * delta, // HOW TO DEAL WITH THIS INTEGRATION ISSUE; NOT ADDITIVE IN NEW MODEL
        roll: sample.roll, //sample.roll + (next.roll - sample.roll) * delta,
        pitch: sample.pitch //sample.pitch + (next.pitch - sample.pitch) * delta,
      }
    }
  }
})

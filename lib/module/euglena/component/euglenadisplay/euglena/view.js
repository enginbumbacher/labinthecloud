define((require) => {
  ...
  const colmat = new THREE.MeshLambertMaterial({ color: 0x0000ff });
  const wfmat = new THREE.MeshLambertMaterial({ color: 0xffffff });
  // wfmat.wireframe = true;
  const tube = THREE.SceneUtils.createMultiMaterialObject(tubeGeom, [colmat]);
  const leftCap = THREE.SceneUtils.createMultiMaterialObject(sphereGeom, [wfmat]);
  ...
  return class EuglenaView extends EventDispatcher {
    ...
    update(lights, dT, model, track = false) {

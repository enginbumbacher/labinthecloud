...
class ComponentManager extends Controller {
  ...
  initialize(euglena) {
    euglena.controllerState.component = {
      reactionStrength: this._model.get('reactionStrength') + (Math.random() * 2 - 1) * this._model.get('reactionStrength_delta'),
      v: this._model.get('v') + (Math.random() * 2 - 1) * this._model.get('v_delta'),
      roll: this._model.get('roll') + (Math.random() * 2 - 1) * this._model.get('roll_delta')
    }
  }
  ...
  update(state) {
    ...
    out.dRoll = state.controllerState.component.roll * state.dT;
    ...
    out.dYaw = state.controllerState.component.reactionStrength * intensity + (Math.random() * 2 - 1) * this._model.get('randomness');

...
export default class ExperimentForm extends Form {
  constructor() {
    ...
    var availableCategories = {'default_choice': 'please choose one'}
    if (Globals.get('AppConfig.experiment.formOptions').length) {
      Globals.get('AppConfig.experiment.formOptions').forEach((expOption) => {
        if (Object.keys(defaults).indexOf(expOption)>-1) {
          availableCategories[expOption] = defaults[expOption]
        }
      })
    } else {

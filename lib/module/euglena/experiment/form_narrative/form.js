...
const defaultExpOptions = {
  'exp_category': {
    'default_choice': 'please choose one',
    'no_light': 'Show no light',
    'fix_dir': 'Single light direction',
    'simple_seq': 'Light sequence',
    'complex_seq': 'Light sequence after light from all directions'
  },
  'exp_times': [4,6]
}
...
export default class ExperimentForm extends Form {
  constructor() {
    ...
    var availableCategories = defaultExpOptions['exp_category'];
    var availableTimes = defaultExpOptions['exp_times'];
    if (Globals.get('AppConfig.experiment.formOptions')) {
      if (Object.keys(Globals.get('AppConfig.experiment.formOptions')).indexOf('exp_categories') > -1) {
        Object.keys(availableCategories).forEach((expOption) => {
          if (expOption != 'default_choice' && Globals.get('AppConfig.experiment.formOptions')['exp_categories'].indexOf(expOption)==-1) {
            delete availableCategories[expOption];// = defaults[expOption]
          }
        })
      }

      if (Object.keys(Globals.get('AppConfig.experiment.formOptions')).indexOf('exp_times') > -1) {
        availableTimes.forEach((expTime) => {
          if (Globals.get('AppConfig.experiment.formOptions')['exp_times'].indexOf(expTime)==-1) {
            availableTimes.splice(availableTimes.indexOf(expTime),1);
          }
        })
      }
    }
    ...
    if (availableTimes.length == 1) {
      var options_firstlight = {'default_choice': 'please choose one', 'fix_dir_constant_10': '10%', 'fix_dir_constant_20': '20%', 'fix_dir_constant_40': '40%', 'fix_dir_constant_60': '60%', 'fix_dir_constant_80': '80%', 'fix_dir_constant_100': '100%',
                'simple_seq_around_simple_seq_alternate_left': 'from the left', 'simple_seq_alternate_top': 'from the top', 'simple_seq_2alternates_topleft': 'diagonal: top-left and bottom-right', 'simple_seq_2alternates_leftright': 'sides: left-right and top-bottom', 'simple_seq_2alternates_alldir': 'all directions and no light',
                'complex_seq_left': 'from the left', 'complex_seq_top': 'from the top'}
    } else {
      var options_firstlight = {'default_choice': 'please choose one', 'fix_dir_constant_10': '10%', 'fix_dir_constant_20': '20%', 'fix_dir_constant_40': '40%', 'fix_dir_constant_60': '60%', 'fix_dir_constant_80': '80%', 'fix_dir_constant_100': '100%',
                'fix_dir_increase_fix_dir_decrease_4': 'in 4 steps', 'fix_dir_increase_fix_dir_decrease_6': 'in 6 steps',
                'simple_seq_around_simple_seq_alternate_left': 'from the left', 'simple_seq_alternate_top': 'from the top', 'simple_seq_2alternates_topleft': 'diagonal: top-left and bottom-right', 'simple_seq_2alternates_leftright': 'sides: left-right and top-bottom', 'simple_seq_2alternates_alldir': 'all directions and no light',
                'complex_seq_left': 'from the left', 'complex_seq_top': 'from the top'}
    }

    if (availableTimes.length == 1) {
      var options_secondlight = {'default_choice': 'please choose one', 'simple_seq_alternate_right': 'from the right', 'simple_seq_alternate_bottom': 'from the bottom', 'simple_seq_alternate_nolight': 'no light', 'complex_seq_nolight': 'no light', 'complex_seq_left_right': 'from the right', 'complex_seq_top_bottom': 'from the bottom'}
    } else {
      var options_secondlight = {'default_choice': 'please choose one', 'simple_seq_alternate_right': 'from the right', 'simple_seq_alternate_bottom': 'from the bottom', 'simple_seq_alternate_nolight': 'no light',
    'simple_seq_around_simple_seq_2alternates_4': '4 steps','simple_seq_around_simple_seq_2alternates_6': '6 steps', 'complex_seq_nolight': 'no light', 'complex_seq_left_right': 'from the right', 'complex_seq_top_bottom': 'from the bottom'}
    }
      ...
      modelData: {
        ...
        fields: [
          ...
          ExpProtocol.create({
            ...
            defaultValue: 'default_choice',
            classes:[],
            options: {'default_choice': 'please choose one', 'fix_dir_constant': 'Set a specific brightness', 'fix_dir_increase': 'Gradually increase the brightness', 'fix_dir_decrease': 'Gradually decrease the brightness',
            'simple_seq_around': 'Make the light go around', 'simple_seq_alternate': 'Alternate between two single directions', 'simple_seq_2alternates': 'Alternate between two directions with multiple lights',
          ...
          ExpProtocol.create({
            ...
            defaultValue: 'default_choice',
            classes:[],
            options: {'default_choice': 'please choose one', 'fix_dir_alldir': 'from all directions', 'fix_dir_left': 'from the left', 'fix_dir_top': 'from the top', 'fix_dir_right': 'from the right','fix_dir_bottom': 'from the bottom',
                        'simple_seq_10': '10%', 'simple_seq_20': '20%', 'simple_seq_40': '40%', 'simple_seq_60': '60%', 'simple_seq_80': '80%', 'simple_seq_100': '100%',
                        'complex_seq_alldir_10': '10%', 'complex_seq_alldir_40': '40%', 'complex_seq_alldir_80': '80%', 'complex_seq_alldir_100': '100%'},
            validation: {}
          }),
          ExpProtocol.create({
            id: "exp_firstlight",
          ...
          ExpProtocol.create({
            ...
            defaultValue: 'default_choice',
            classes:[],
            options: {'default_choice': 'please choose one', 'simple_seq_alternate_4': '4 steps', 'simple_seq_alternate_6': '6 steps', 'complex_seq_10': '10%', 'complex_seq_40': '40%', 'complex_seq_60': '60%'},
            validation: {}
          })
        ],
        buttons: buttons
    ...
    this.chainOfActivation = {
      ...
      'veryShortChainOfActivation': {"exp_category": "exp_procedure", "exp_procedure": "exp_holdconstant"}
    };
  ...
  getLightConfiguration() {
    ...
    switch(this.chainState) {
      ...
      break;
      case 'veryShortChainOfActivation':
        configState = defaultCounter < 4 ? true : false;
      break;
      case 'final':
        configState = true;
      break;
    ...
    let numPanels = this.availableTimes.length > 1 ? 6 : this.availableTimes[0];
    var lightConfig = {}
    ...
    if (evt.data.field._model._data.id == 'exp_category') {
      ...
      switch (evt.data.delta.value) {
        ...
        case 'fix_dir_increase':
          this._modifyOptions(field_firstlight,evt.data.delta.value);
          this.chainState = this.availableTimes.length>1 ? 'shortChainOfActivation' : 'veryShortChainOfActivation';
        break;
        case 'simple_seq_around':
        case 'simple_seq_2alternates':
          this._modifyOptions(field_firstlight, evt.data.delta.value);
          this.chainState = this.availableTimes.length>1 ? 'mediumChainOfActivation' : 'shortChainOfActivation';
        break;
        case 'simple_seq_alternate':
          this._modifyOptions(field_firstlight, evt.data.delta.value);
          this.chainState = this.availableTimes.length>1 ? 'fullChainOfActivation' : 'mediumChainOfActivation';
        break;

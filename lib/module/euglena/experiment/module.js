    ...
    init() {
      if (Globals.get('AppConfig.experiment')) {
        ...
        return promise.then(() => {
          ...
          this._tabView.addChild(new DomView("<div id='expProtocol__title'> Experiment Protocol: </div>"));
          ...
          if (Globals.get('AppConfig.experiment.experimentForm')=='table') {
            ...
            for (var numPanels = 0; numPanels < 4; numPanels++) {
              ...
              let _timeDisplay = new DomView('<div>' + (numPanels + 1) + '.<br>' + (Globals.get('AppConfig.experiment.maxDuration') * numPanels / 4.0) + ' - ' + (Globals.get('AppConfig.experiment.maxDuration') * (numPanels + 1 ) / 4.0) + ' sec </div>');

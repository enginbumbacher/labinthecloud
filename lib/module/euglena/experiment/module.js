...
import ExperimentTableForm from './form_table/form';
import ExperimentNarrativeForm from './form_narrative/form';
...
import ServerInterface from './serverinterface/module';
import Timer from 'core/util/timer';
import EugUtils from 'euglena/utils';
import ExperimentReporter from './reporter/reporter';
import './style.scss';
...
class ExperimentModule extends Module {
  constructor(ctx) {
    ...
    Utils.bindMethods(this, [
      ...
      '_hookCurrentExperiment'
    ]);

    Globals.get('Relay').addEventListener('AppPhase.Change', this._onPhaseChange);
    HM.hook(`${this.context.app}.CurrentExperiment`, this._hookCurrentExperiment);
  }
    ...
    if (Globals.get('AppConfig.experiment')) {
      ...
      return promise.then(() => {
        ...
        if (Globals.get('AppConfig.experiment.experimentForm')=='table') {
          ...
          // 5. Create the dryRun specifications
          this._expVizLights = []
          for (var numPanels = 0; numPanels < this.maxPanels; numPanels++) {
            ...
            let _countDisplay = new DomView('<div>' + (numPanels + 1) + '.<br>');
            let _timeDisplay = new DomView('<div id=exp_viz__time>' + (Globals.get('AppConfig.experiment.maxDuration') / this.maxPanels) + ' sec </div>');
            let _brightnessDisplay = new DomView("<div id='exp_viz__brightness'> Light </div>");
            newLights.view().addChild(_countDisplay, '.light-display__content');
  ...
  _hookPanelContents(list, meta) {
    if (meta.id == "interactive" && meta.app == this.context.app) {
      ...
      if (id == "_new") {
        ...
        .then((data) => {
          this._configForm.import(
            Object.assign({},
              data.expForm,
              { lights: data.configuration, name: data.name }
            )
          );
          return data;
        }).then((data) => {
  ...
  _onConfigChange(evt) {
    if (Globals.get('AppConfig.experiment.experimentForm') == 'table'){
      ...
      var lightLevels = {'-1':'', '0':'off', '10':'10%', '20':'20%', '25':'25%','40':'40%', '50':'50%', '60':'60%', '75': '75%', '80':'80%', '100':'100%'};

      // Render the relevant number of panels
      let numPanels = lightConfig['brightness'].length ? lightConfig['brightness'].length : 1;

      for (let panel = 0; panel < this.maxPanels; panel++) {
        if (panel < numPanels) {
          this._expViz._children[0]._children[panel].show()
          ...
          this._expViz._children[0]._children[panel].$el.find('#exp_viz__time').html(Math.round(60 / numPanels) + ' sec')
        } else {
          this._expViz._children[0]._children[panel].hide() // Hide the remaining panels
        }
      }
    }
  }
  ...
  _onRunRequest(evt) {
    ...
    this._configForm.validate().then((validation) => {
      ...
      if (Globals.get('AppConfig.experiment.revertToLast')) {
  ...
  _onAggregateRequest(evt) {
    EugUtils.getLiveResults(this._history.export().exp_history_id, this.context.app).then((results) => {
  ...
  destroy() {
    ...
    HM.unhook(`${this.context.app}.CurrentExperiment`, this._hookCurrentExperiment);
    Globals.removeEventListener('Model.Change', this._onGlobalsChange);
    Globals.get('Relay').removeEventListener('ExperimentServer.Update', this._onServerUpdate);
    Globals.get('Relay').removeEventListener('ExperimentServer.Results', this._onServerResults);
    Globals.get('Relay').removeEventListener('ExperimentServer.Failure', this._onServerFailure);
    ...
    Globals.set('State', null);
    Globals.set('currentExperiment', null);

    let destruction = [];
    if (this._history) destruction.push(this._history.destroy());
    if (this._dryRunLights) destruction.push(this._dryRunLights.destroy());
    if (this._dryRunBulbs) destruction.push(this._dryRunBulbs.destroy());
    if (this._expVizLights) destruction = destruction.concat(this._expVizLights.map((l) => l.destroy()));
    if (this._configForm) destruction.push(this._configForm.destroy());
    return Promise.all(destruction.concat(super.destroy()));
  }

  _hookCurrentExperiment(subj, meta) {
    return this._configForm.state == 'new' ? null : Globals.get('currentExperiment');
  }
}

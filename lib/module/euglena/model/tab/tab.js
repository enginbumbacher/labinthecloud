...
class ModelTab extends Component {
  constructor(settings = {}) {
    ...
    if (this._model.get('modelType') == 'blockly' || this._model.get('modelType') == 'mech') {
      ...
      if (this._model.get('parameters').K) { paramOptions['reaction'] = Object.keys(this._model.get('parameters').K.options) }
      if (this._model.get('parameters').v) { paramOptions['motor'] = Object.keys(this._model.get('parameters').v.options) }

      if (this._model.get('parameters').omega) { paramOptions['roll'] = Object.keys(this._model.get('parameters').omega.options)
  ...
  _onConfigChange(evt) {
    ...
    if (evt.name === 'Form.FieldChanged') {
      ...
      else if (evt.currentTarget._model._data.modelType == 'blockly' || evt.currentTarget._model._data.modelType == 'mech'){
      ...
      if (id != '_new') {
        ...
        Utils.promiseAjax(`/api/v1/EuglenaModels/${id}`).then((data) => {
          ...
          if (this._model._data.modelType == 'blockly') {
            Globals.get('Relay').dispatchEvent('ModelSideTab.Load', data.blocklyXml);
          ...
          } else if (this._model._data.modelType == 'mech') {
            Globals.get('Relay').dispatchEvent('ModelSideTab.Load', 'THIS CONTAINS THE VARIABLES TO BE LOADED IN THE FORM');
  ...
  _onSimulateRequest(evt) {
    ...
    } else if (this._model.get('modelType') == 'mech') {
      // DO SOMETHING HERE to modify conf.
      // let modelSideTabData = this._extractMechForm()
      // saveData.configuration = $.extend(saveData.configuration,modelSideTabData); // extend saveData.configuration
      sensorConfigJSON = JSON.stringify(this.bodyConfigurations.getActiveSensorConfiguration());
      saveData = $.extend(saveData,{sensorConfigJSON: sensorConfigJSON})
    }
  ...
  _onNameSubmit(evt) {
    ...
    if (this._model.get('modelType') == 'blockly') {
      ...
      // Find a way to extract the variables of the side tab.
      // this._extractMechForm() to extend conf
    }
    ...
    Globals.get('Logger').log({
      ...
      category: "model",
      data: {
        configuration: blocklyData ? $.extend(this._form.export(), {sensorConfigJSON: sensorConfigJSON, jsCode: blocklyData.jsCode}) : conf ,
        modelType: this._model.get('modelType'),
        name: this._nameForm.export().name
      }
    })

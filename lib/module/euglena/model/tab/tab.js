...
class ModelTab extends Component {
  constructor(settings = {}) {
    ...
    this._history = ModelHistoryForm.create({
      ...
      tabStaticHistory: this._model.get('modelHistory')
    });
    ...
    if (this._model.get('modelType') == 'blockly') {
      this.bodyConfigurations = HM.invoke('BlocklyConfig.Build', null, {
        form: this._form
      });
  ...
  _onHistorySelectionChange(evt) {
    if (evt.name === 'Blockly.Changed') {
      ...
      console.log(evt.currentTarget.export());
  ...
  _onConfigChange(evt) {
    ...
    if (evt.name === 'Form.FieldChanged') {
      if (evt.data.field.id() === 'opacity') {
        this.bodyConfigurations.setBodyOpacity(evt.data.delta.value)
      }

      else if (evt.currentTarget.modelType() == 'blockly'){
  ...
  triggerSave() {
    let savePromise = new Promise((resolve, reject) => {
      let _spCancel = (evt) => {
        this._nameForm.view().removeEventListener('ModelSave.Cancel', _spCancel);
        this.removeEventListener('ModelTab.ModelSaved', _spConfirm);
        this.removeEventListener('ModelTab.ModelSaveError', _spCancel);
        reject(evt.type == 'ModelSave.Cancel' ? 'Save canceled' : evt.data.error);
      }
      let _spConfirm = (evt) => {
        this._nameForm.view().removeEventListener('ModelSave.Cancel', _spCancel);
        this.removeEventListener('ModelTab.ModelSaved', _spConfirm);
        this.removeEventListener('ModelTab.ModelSaveError', _spCancel);
        resolve(evt.data.model);
      }
      this._nameForm.view().addEventListener('ModelSave.Cancel', _spCancel);
      this.addEventListener('ModelTab.ModelSaved', _spConfirm);
      this.addEventListener('ModelTab.ModelSaveError', _spCancel);
      this._onSaveRequest(null);
    })
  ...
  _onNameSubmit(evt) {
    ...
    this._nameForm.validate().then((validation) => {
      ...
      this.dispatchEvent('ModelTab.ModelSaved', {
        model: model
      })
    }).catch((e) => {
      this.dispatchEvent('ModelTab.ModelSaveError', {
        error: e
      })
    })
  }
  ...
  _onAggregateRequest(evt) {
    EugUtils.getModelResults(Globals.get('currentExperiment.id'), this._currentModel, this._model.get('app')).then((results) => {
  ...
  destroy() {
    ...
    Globals.get('Relay').removeEventListener('Blockly.Changed', this._onHistorySelectionChange);
    Globals.get('Relay').removeEventListener('Blockly.Changed', this._onConfigChange);
    return Promise.all([
      this._form.destroy(),
      this._history.destroy(),
      this._nameForm.destroy(),
    ]);
  }

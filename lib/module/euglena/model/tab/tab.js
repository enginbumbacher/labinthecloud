define((require) => {
  ...
  class ModelTab extends Component {
        ...
        if (this._lastSimulationId) {
          ...
          return Utils.promiseAjax('/api/v1/Results', {
            ...
            data: JSON.stringify({
              ...
              euglenaModelId: model.id,
              experimentId: Globals.get('currentExperiment').id,
              initialization: this._initializeModelEuglena(model.count)
            }),
            contentType: 'application/json'
          })
        }
    ...
    _onAggregateRequest(evt) {
      EugUtils.getModelResults(Globals.get('currentExperiment.id'), this._history.export().model_history_id).then((results) => {

  ...
  class ModelTab extends Component {
    constructor(settings = {}) {
      ...
      if (this._model.get('modelType') == 'blockly') {
        ...
        this.bodyConfigurations = BodyConfigurations.create({bodyConfigurationName: initialBody['bodyConfigurationName'],
    ...
    _onConfigChange(evt) {
      ...
      if (evt.name == 'Form.FieldChanged') {
        if (evt.data.field._model._data.id == 'opacity') {
          let opacity = parseInt(evt.data.delta.value.substr(evt.data.delta.value.indexOf('_')+1)) / 100
        ...
        if (id != '_new') {
          ...
          Utils.promiseAjax(`/api/v1/EuglenaModels/${id}`).then((data) => {
            ...
            if (this._model._data.modelType == 'blockly') {
              ...
              this.bodyConfigurations.setActiveConfiguration(data.configuration.bodyConfigurationName);
            }

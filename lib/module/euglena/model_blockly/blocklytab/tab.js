...
class ModelingDataTab extends Component {
  constructor(settings = {}) {
    ...
    if (!(Globals.get('AppConfig.system.expModelModality')==='justbody')) {
      ...
      Globals.get('Relay').addEventListener('InteractiveTabs.TabChange', this._onToggleRequest)
    }
    ...
    let toolbox = $('#toolbox');
    let lab = Globals.get('AppConfig');
    if (lab.model.blocks.turning.turn_at_1sensor.allow) {
      toolbox.find('category[name="Turning"]').append(`
        <block type="turn_at_1sensor" id="turn_at_1sensor" disabled='false' max_use='${lab.model.blocks.turning.turn_at_1sensor.maxUse}' sensors='one' equivalence='turn_at_2sensors'>
    ...
    if (lab.model.blocks.forward.move_normal.allow) {
      toolbox.find('category[name="Forward"]').append(`
    ...
    if (lab.model.blocks.rotation.roll_normal.allow) {
      toolbox.find('category[name="Rotation"]').append(`
    ...
    if (lab.model.blocks.additional.see_light_quantity.allow) {
      toolbox.find('category[name="Additional Blocks"]').append(`
  ...
  _onToggleRequest(evt) {
    ...
    if (!this._model.get('open') && this.workspace) {
    ...
    } else if (evt.data.tab && evt.data.tab._model.get('type') == 'blockly') {
      // using _model here is a hack. ip_tabs should be reconfigured to accept
      // either tabConf objects of Tab instances, and the ModelTab should override Tab
      this._model.toggle();
      this._view.toggle(this._model.get('open'));
...
/*
  ...
  destroy() {
    Globals.get('Relay').removeEventListener('ModelingTab.ToggleRequest', this._onToggleRequest)
    Globals.get('Relay').removeEventListener('InteractiveTabs.TabChange', this._onToggleRequest)
    Globals.get('Relay').removeEventListener('AppPhase.Change', this._onPhaseChange)
    Globals.get('Relay').removeEventListener('Blockly.Load', this._onBlocklyLoad)
    Globals.get('Relay').removeEventListener('Notifications.Remove',this._listenBlocklyEvents)
    Globals.get('Relay').removeEventListener('ModelingTab.TransitionEnd',this._listenBlocklyEvents)
    ...
    Globals.get('Relay').removeEventListener('Body.Change', this._onBodyChange);
    Globals.get('Relay').removeEventListener('Toolbox.Update',this._updateToolbox);

    Globals.set('blocklyLoaded', null);

    let toolbox = $('#toolbox');
    toolbox.find('category[name="Turning"]').html('');
    toolbox.find('category[name="Forward"]').html('');
    toolbox.find('category[name="Rotation"]').html('');
    toolbox.find('category[name="Additional Blocks"]').html('');

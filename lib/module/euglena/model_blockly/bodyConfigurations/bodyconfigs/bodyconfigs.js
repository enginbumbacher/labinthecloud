...
const defaults = {
  ...
  options: {
    type: ['type_1side', 'type_1middle', 'type_2sensors', 'type_eyespot'],
...
class BodyConfigurations extends Component {
  constructor(settings = {}) {
    ...
    this.imgPath = 'https://s3.us-east-2.amazonaws.com/euglena-dev/baseImages/bodyImgs/'
    this._importAllImages();

    this.setShapeAutomatically = false; // needed to have type of sensor influence shape.
    if (Object.keys(settings.modelData.initialConfig).indexOf('sensorType') >-1) {
      if (settings.modelData.initialConfig.sensorType.match('eyespot')) {
        this.setShapeAutomatically = true;
      }
    }
  ...
  initializeActiveConfiguration(initialConfig) {
    this.initializeMapping = {'type': 'sensorType', 'shape': 'sensorShape', 'position': 'sensorPosition', 'reaction': 'k', 'motion': 'motion', 'motor': 'v', 'roll': 'roll' }
    ...
    Object.keys(this.initializeMapping).forEach(params => {
      if (Object.keys(initialConfig).indexOf(this.initializeMapping[params]) >-1) {
        ...
        if (params === 'shape') {
          ...
          this.setActiveConfiguration('motor', null)
        }
      }
    })
  }
    ...
    switch (configType) {
      case 'type':
        ...
        if (currNumSensors != prevNumSensors) {
          Globals.get('Relay').dispatchEvent('Body.Change', {numSensors: currNumSensors, sensorType: configName})
        } else if (configName.match('1side|eyespot')) {
          Globals.get('Relay').dispatchEvent('Body.Change', {numSensors: currNumSensors, sensorType: configName})
        }

        this.activeConfigs[configType] = configName;

        if (this.setShapeAutomatically) {
          if (this.activeConfigs[configType].match('1middle|eyespot')) {
            this.activeConfigs['shape'] = 'shape_100'

          } else {
            this.activeConfigs['shape'] = 'shape_50'
          }
        }
  ...
  getActiveSensorConfiguration() {
    ...
    */
    ...
    if (this.activeConfigs['type'].match('eyespot|1middle')) {
      JSONConfig['sensor_1']['sensorPosition'].y = 0;
    } else {
    ...
    JSONConfig['spotPositions'] = [];
    if (this.activeConfigs['type'].match('eyespot')) {
      JSONConfig['spotPositions'][0] = {};
      JSONConfig['spotPositions'][0].z = 1 - 2 * parseInt(this.activeConfigs['position'].substr(this.activeConfigs['position'].indexOf('_')+1)) / 100;
      JSONConfig['spotPositions'][0].y = -1
    }
  ...
  _importAllImages() {
    ...
    if (this.paramOptions.shape) {
      ...
      sensorOptions['shape'] = defaults['options']['shape']
    }
    ...
    if(Object.keys(this.paramOptions).indexOf('motor')>-1) {
      this.paramOptions['motor'].forEach(strength => {
        fileName = this.modelForm == 'mechanistic'? this.modelForm + '_' + 'motor' : this.modelForm + '_' + 'motor' + '_' + 'strength' + '_' + strength.substr(strength.indexOf('_')+1);
        this.view()._addLayer(fileName, this.imgPath);
      })
    } else {
    ...
    if (Object.keys(this.paramOptions).indexOf('roll')>-1) {
      this.paramOptions['roll'].forEach(strength => {
        fileName = this.modelForm == 'mechanistic'? this.modelForm + '_' + 'roll' : this.modelForm + '_' + 'roll' + '_' + 'strength' + '_' + strength.substr(strength.indexOf('_')+1);
        this.view()._addLayer(fileName, this.imgPath);
      })
  ...
  _getDivId(variableName) { //variableName is either 'type','shape','position','reaction','motor','roll','motion'
    ...
    */
    if (variableName.match('type|shape|position')) {
      ...
      return this.modelForm == 'mechanistic'? this.modelForm + '_' + variableName : this.modelForm + '_' + variableName + '_strength_' + this.activeConfigs[variableName].substr(this.activeConfigs[variableName].indexOf('_')+1)
    }
  }

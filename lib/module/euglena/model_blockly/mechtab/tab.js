...
const defaults = {
  ...
  options: {
    ...
    sensorType: ['type_1side', 'type_eyespot'],
    numSensors: [1,2]
...
class ModelingDataTab extends Component {
  constructor(settings = {}) {
    ...
    this.imgPath = 'https://s3.us-east-2.amazonaws.com/euglena-dev/baseImages/mechImgs/'
    ...
    // }
    this.activeConfigs = {
      channelOpeningAmount: defaults['values']['channelOpeningAmount'],
      turnForward: defaults['values']['turnForward'],
      turnDirection: defaults['values']['turnDirection'],
      numSensors: defaults['values']['numSensors'],
      sensorType: defaults['values']['sensorType']
    }
  ...
  _onPhaseChange(evt) {
    if (evt.data.phase == "login" || evt.data.phase == "login_attempted") {
      if (!Globals.get('mechLoaded')) {
        ...
        var tmpEvt = {}
        tmpEvt['data'] = {'numSensors': this.activeConfigs['numSensors'], 'sensorType': this.activeConfigs['sensorType']}
        this._onBodyChange(tmpEvt)
        this._updateSignalText(this._model.get('parameters'), 'constructor', ['signalThresh','signalAdaptSpeed','signalThreshPosNeg','signalThreshAllNone','reactionStrength'])
  ...
  _onConfigChange(evt) {
    ...
    if (document.getElementById(evt.data.field._model._data.id)) {
      ...
      this._updateSignalText(updateText, 'configChange', ['signalThresh','signalAdaptSpeed','signalThreshPosNeg','signalThreshAllNone','reactionStrength'])
  ...
  _onMechLoad(evt) {
    ...
    if (evt.data) {
     ...
     this._signalActivateForm.import(evt.data.config, ['signalThresh','signalThreshAllNone','signalAdaptSpeed']).then(() => {
       this._signalActivateForm.addEventListener('Form.FieldChanged', this._onConfigChange)
     })
     ...
     if (evt.data.sensorType) {
       ...
       var tmpEvt = {}
       tmpEvt['data'] = {'numSensors': numSensors ? numSensors : 1, 'sensorType': this.activeConfigs['sensorType']}

       this._onBodyChange(tmpEvt)
     }

     this._updateSignalText(evt.data.config,'modelLoad', ['signalThresh','signalAdaptSpeed','signalThreshPosNeg','signalThreshAllNone','channelOpeningAmount','reactionStrength'])
  ...
  _importAllImages() {
    ...
    //   }]
    var imgNames = []
    defaults['options']['numSensors'].forEach((elem_sensors) => {
      defaults['options']['sensorType'].forEach((elem_sensorType) => {
        defaults['options']['channelOpeningAmount'].forEach((elem_channel) => {
          defaults['options']['turnForward'].forEach((elem_forward) => {
            defaults['options']['turnDirection'].forEach((elem_direction) => {
              imgNames.push({
                divId: 'model_images',
                imgId: this.mechanism + '_' + elem_sensors + '_' + elem_sensorType + '_signal-activation_' + elem_channel + '_behavior-generation_' + elem_forward + '_' + elem_direction
              })
            })
          })
        })
  ...
  hideImg() { //configId is the ID given to the corresponding div, as assigned in _importAllImages; it is created based on the filename
    let imgId = this.mechanism + '_' + this.activeConfigs['numSensors'] + '_' + this.activeConfigs['sensorType'] + '_' + 'signal-activation_' + this.activeConfigs['channelOpeningAmount'] + '_behavior-generation_' + this.activeConfigs['turnForward'] + '_' + this.activeConfigs['turnDirection'];
  ...
  _onBodyChange(evt) {
    ...
    if (this.activeConfigs['numSensors'] != evt.data.numSensors) {
      this.hideImg();
      this.activeConfigs['numSensors'] = evt.data.numSensors;
      this.showImg();
    } else if (this.activeConfigs['sensorType'] != evt.data.sensorType) {
      this.hideImg();
      this.activeConfigs['sensorType'] = evt.data.sensorType
      this.showImg();
    }

...
class ModelingDataTab extends Component {
  constructor(settings = {}) {
    ...
    //   'imgs_signal-processing_signalReleaseAmount': 'signal-processing_' + defaults['signalReleaseAmount'],
    //   'imgs_behavior-generation_turnForward': 'behavior-generation_' + defaults['turnForward']
    // }
    this.activeConfigs = {
      signalReleaseAmonut: defaults['signalReleaseAmount'],
      turnForward: defaults['turnForward']
    }
  ...
  _importAllImages() {
    ...
    var imgNames = [{
        divId: 'model_images',
        imgId: 'signal-activation_amount_constant_behavior-generation_forward_0'
      }, {
        divId: 'model_images',
        imgId: 'signal-activation_amount_constant_behavior-generation_forward_1'
      },{
        divId: 'model_images',
        imgId: 'signal-activation_amount_proportional_behavior-generation_forward_0'
      },{
        divId: 'model_images',
        imgId: 'signal-activation_amount_proportional_behavior-generation_forward_1'
      }]
  ...
  hideImg() { //configId is the ID given to the corresponding div, as assigned in _importAllImages; it is created based on the filename
    let imgId = 'signal-activation_' + this.activeConfigs['signalReleaseAmount'] + '_behavior-generation_' + this.activeConfigs['turnForward'];
      ...
      if (paramName) {
        ...
        switch(paramName) {
          case 'signalReleaseAmount':
            ...
            // this.hideImg(this.activeImgs['imgs_signal-activation_signalReleaseAmount'])
            // this.hideImg(this.activeImgs['imgs_signal-processing_signalReleaseAmount'])
            this.hideImg();

            // show new images
            imgName = 'signal-activation_' + imgAppendix;
            let imgName2 = 'signal-processing_' + imgAppendix;
            // this.showImg(imgName);
            // this.showImg(imgName2);
            // this.activeImgs['imgs_signal-activation_signalReleaseAmount'] = imgName;
            // this.activeImgs['imgs_signal-processing_signalReleaseAmount'] = imgName2;
            this.activeConfigs['signalReleaseAmount'] = imgAppendix;
            this.showImg();
          ...
          case 'turnForward':
            ...
            // show new image
            imgName = 'behavior-generation_' + imgAppendix;
            //this.showImg(imgName);
    ...
    Object.keys(updateData).forEach(fieldId => {
      ...
      if (textField) {
        ...
        if (fieldId.match('signalAdapt$')) {
          ...
          2. When light hits one of the photoreceptors: Only when this light is brigher than <span class="modeling__interface__textInserts" id="signalThresh"> a certain amount of the maximum brightness </span>, the photoreceptor releases Signal Molecules. <span class='modeling__interface__textInserts' id='signalRelease' style="display:none"> </span>
          ...
          if (fieldVal.match('constant')) {
            document.getElementById("signal-processing__baselineText__signalReleaseAmount").innerHTML = "3. So, here I will say sth about how the gates are opened. <span class='modeling__interface__textInserts' id='signalReleaseAmount'> Opened in a constant manner </span>. If not, it won't release them."
          } else {
            document.getElementById("signal-processing__baselineText__signalReleaseAmount").innerHTML = "3. So, here I will say sth about how the gates are opened.  <span class='modeling__interface__textInserts' id='signalReleaseAmount'> Opened in a proportional manner </span>. If not, it won't release them."
          }

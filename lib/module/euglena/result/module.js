...
export default class ResultsModule extends Module {
  ...
  _hookPanelContents(subject, meta) {
    if (meta.id == "result" && meta.app == this.context.app) {
  ...
  _onExperimentLoaded(evt) {
    ...
    if (evt.data.directModelComparison) { // Load experiment into the second video view for model comparison when activated for the first time
      ...
      if (evt.data.experiment.id != '_new') {
        ...
        this._view.clear();
        this._view.disableVideoControls();
      }
    } else {
    ...
    */
    ...
    if (evt.data.model.id != '_new' && Globals.get('currentExperiment')) {
      if (!(evt.data.tabId == this._currentModel && this._lastModelResult[`model_${evt.data.tabId}`] &&
        ...
        EugUtils.getModelResults(Globals.get('currentExperiment').id, evt.data.model, this.context.app).then((results) => {
  ...
  _onModelResultsRequest(evt) {
    if (evt.data.tabId == 'none') {
      ...
      if (currModel_a) {
        ...
        if (!Globals.get('AppConfig.system.modelModality').match('create')) { Globals.get('Relay').dispatchEvent('Model.AutomaticSimulate',{tabId: 'a'}); }
      }
      ...
      if (currModel_b) {
        ...
        if (!Globals.get('AppConfig.system.modelModality').match('create')) { Globals.get('Relay').dispatchEvent('Model.AutomaticSimulate',{tabId: 'b'}); }
      }
      ...
      if (currModel) {
        ...
        if (!Globals.get('AppConfig.system.modelModality').match('create')) { console.log('automatic simulate'); Globals.get('Relay').dispatchEvent('Model.AutomaticSimulate',{tabId: evt.data.tabId}); }
      }
  ...
  destroy() {
    Globals.get('Relay').removeEventListener('Experiment.Loaded', this._onExperimentLoaded);
    Globals.get('Relay').removeEventListener('EuglenaModel.Loaded', this._onModelLoaded);
    ...
    Globals.get('Relay').removeEventListener('EuglenaModel.directModelComparison', this._onExperimentLoaded);
    HM.unhook('Panel.Contents', this._hookPanelContents);
    Globals.set('currentExperimentResults', null);
    Globals.set('currentLightData', null);
    Globals.set('directModelComparison', null);
    return Promise.all([this._view.destroy(), super.destroy()]);
  }
}

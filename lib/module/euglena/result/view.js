...
export default class ResultsView extends DomView {
  constructor(tmpl) {
    super(tmpl || Template);
    Utils.bindMethods(this, ['_onTick', '_onVisualizationChange', '_onModelChange', 'activateModelComparison', '_moveTabs','_resetModelSelect','disableVideoControls','enableVideoControls']);

    this._visualResult = VisualResult.create();
    this._visualResult.addEventListener('VisualResult.Tick', this._onTick);
    this._graphs = Globals.get('AppConfig.visualization') ?  Globals.get('AppConfig.visualization.visualizationTypes').map((visConf) => {
  ...
  disableVideoControls() {
    this._visualResult.disableVideoControls();
  }

  enableVideoControls() {
    this._visualResult.enableVideoControls();
  }
  ...
  destroy() {
    Globals.get('Relay').removeEventListener('EuglenaModel.directComparison', this.activateModelComparison);
    let destruction = [];
    destruction.push(this._visualResult.destroy());
    if (this._visualResult_2) destruction.push(this._visualResult_2.destroy());
    destruction = destruction.concat(this._graphs.map((g) => g.destroy()));
    destruction.push(this._modelSelect.destroy());
    destruction.push(this._visSelect.destroy());
